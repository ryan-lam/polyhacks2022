<template>
		...
		<nav class="nav" :style="{'background-color': colours[drkmode].maincolour[getPerson] }">
			...
			<div class="nav__item" :style="{'--hoveritem': colours[drkmode].hovercolour[getPerson]}">
				<svg xmlns="http://www.w3.org/2000/svg" height="36px" viewBox="0 0 24 24" width="36px" :fill="colours[drkmode].unhighlight[getPerson]"><path d="M0 0h24v24H0z" fill="none"/><path d="M12 11.55C9.64 9.35 6.48 8 3 8v11c3.48 0 6.64 1.35 9 3.55 2.36-2.19 5.52-3.55 9-3.55V8c-3.48 0-6.64 1.35-9 3.55zM12 8c1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3 1.34 3 3 3z"/></svg>
				<p :style="{'color': colours[drkmode].unhighlight[getPerson]}">My Courses</p>
			</div>
			<div class="nav__item" :style="{'--hoveritem': colours[drkmode].hovercolour[getPerson]}">
				<svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 20 20" height="36px" viewBox="0 0 20 20" width="36px" :fill="colours[drkmode].unhighlight[getPerson]"><g><rect fill="none" height="20" width="20" y="0"/></g><g><g><path d="M16.5,2h-13C2.67,2,2,2.67,2,3.5v3c0,0.65,0.42,1.2,1,1.41v8.59C3,17.33,3.67,18,4.5,18h11c0.83,0,1.5-0.67,1.5-1.5V7.91 c0.58-0.21,1-0.76,1-1.41v-3C18,2.67,17.33,2,16.5,2z M16.5,6.5h-13v-3h13V6.5z M4.5,16.5V8h11v8.5H4.5z"/><rect height="1.5" width="4" x="8" y="10"/></g></g></svg>
				<p :style="{'color': colours[drkmode].unhighlight[getPerson]}">Course Materials</p>
			</div>
			<div class="nav__item" :style="{'--hoveritem': colours[drkmode].hovercolour[getPerson]}">
				...
				<p :style="{'color': colours[drkmode].unhighlight[getPerson]}">Notes</p>
			</div>
			<div class="switch">
				...
				<p :style="{'color': colours[drkmode].unhighlight[getPerson]}">Dark Mode</p>
			</div>
			<div class="nav__item" :style="{'--hoveritem': colours[drkmode].hovercolour[getPerson]}" @click="home()">
				<svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 24 24" height="36px" viewBox="0 0 24 24" width="36px" :fill="colours[drkmode].unhighlight[getPerson]"><g><path d="M0,0h24v24H0V0z" fill="none"/></g><g><path d="M17,8l-1.41,1.41L17.17,11H9v2h8.17l-1.58,1.58L17,16l4-4L17,8z M5,5h7V3H5C3.9,3,3,3.9,3,5v14c0,1.1,0.9,2,2,2h7v-2H5V5z"/></g></svg>
				<p :style="{'color': colours[drkmode].unhighlight[getPerson]}">Logout</p>
			</div>
		...
		<main :style="{'background-color': colours[drkmode].bgcolour}">
			...
			<section class="container">
				...
				<div class="container__courses" v-for="item in courses" :style="{'background-color': colours[drkmode].bgcourse[getPerson]}" @click="goCourse(item.url, item.subject)">
					...
					<p class="container__courses--desc" :style="{'color': colours[drkmode].textcolour[getPerson]}">{{item.description}}</p>
				</div>
				<!-- <div class="container__courses">
					<h2>Sports Trickshots</h2>
				</div>
				<div class="container__courses">
...
export default {
	...
	mounted(){
		...
		const prom = fetch("http://localhost:3000/class", {method: 'GET',}).then(response => response.json()).then(data => {
			console.log(data.data);
			data.data.forEach(element => {
				console.log(element.subject);
				let url = element.subject.split(' ').join('').toLowerCase();
				element["url"] = url;
